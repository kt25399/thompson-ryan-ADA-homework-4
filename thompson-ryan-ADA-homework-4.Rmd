...
```{r}
### Challenge 5
d <- d %>% mutate(lograte = log(rate),
                  logdensity = log(density))
density <- lm(rate ~ density, data = d)
density

ggplot(d, aes(x = density, y = rate)) +
  geom_point() + 
  geom_smooth(formula = y ~ x, method = "lm", se = FALSE)

population <- lm(rate ~ population, data = d)
population

ggplot(d, aes(x = population, y = rate)) + 
  geom_point() + 
  geom_smooth(formula = y ~ x, method = "lm", se = FALSE)

gdp_capita <- lm(rate ~ gdp_capita, data = d)

ggplot(d, aes(x = gdp_capita, y = rate)) + 
  geom_point() + 
  geom_smooth(formula = y ~ x, method = "lm", se = FALSE)

income <- lm(rate ~ income, data = d)
income

d <- d %>% mutate(income = factor(income,
  levels = c("Low income", "Lower middle income", 
             "Upper middle income", "High income")))

ggplot(d, aes(x = income, y = rate)) + 
  geom_point() + 
  geom_smooth(formula = y ~ x, method = "lm", se = FALSE)
```